        ...
        if any(x in b_lower for x in ['isim', 'adı soyadı', 'tarih', 'yaş', 'cinsiyet']):
            ...
            with col2:
                if "isim" in b_lower or "adı soyadı" in b_lower:
                    # Text input - default boş string
                    input_values[baslik] = st.text_input("İsim", key=key_id, label_visibility="collapsed")
                
                elif "tarih" in b_lower:
                    # Date input
                    input_values[baslik] = st.date_input("Tarih", value=datetime.now(), key=key_id, format="DD.MM.YYYY", label_visibility="collapsed")
                    ...
                    # Number input - min_value ile güvenli
                    input_values[baslik] = st.number_input("Yaş", min_value=0, max_value=150, value=0, step=1, key=key_id, label_visibility="collapsed")

# ===============================
# EXPANDER 2: VİTAL BULGULAR (Açık)
# ===============================
with st.expander("💉 VİTAL BULGULAR", expanded=True):
    # Vital alanlarını bul
    vitals_found = []
    for i, baslik in enumerate(headers):
    ...
    for j in range(0, len(vitals_found), 2):
        ...
        for k, col in enumerate(cols):
            if j + k < len(vitals_found):
                ...
                # Sabit değer varsa onu kullan
                default_val = SABIT_DEGERLER.get(b_lower, 0)
                ...
                with col:
                    ...
                    if "ateş" in b_lower:
                        # Ateş ondalıklı
                        input_values[baslik] = st.number_input("Değer", min_value=30.0, max_value=45.0, value=float(default_val), step=0.1, format="%.1f", key=key_id, label_visibility="collapsed")
                    elif "spo2" in b_lower:
                        # SpO2
                        input_values[baslik] = st.number_input("Değer", min_value=0, max_value=100, value=int(default_val), step=1, key=key_id, label_visibility="collapsed")
                    elif "gks" in b_lower:
                        # GKS
                        input_values[baslik] = st.number_input("Değer", min_value=3, max_value=15, value=int(default_val), step=1, key=key_id, label_visibility="collapsed")
                    else:
                        # Tansiyon, nabız
                        input_values[baslik] = st.number_input("Değer", min_value=0, max_value=300, value=int(default_val), step=1, key=key_id, label_visibility="collapsed")
...
with st.expander("📊 SONUÇ & VARDİYA", expanded=False):
    ...
    # Vardiya
    st.markdown("**🕐 Vardiya:**")
    oto_vardiya_default = get_otomatik_vardiya()
    ...
    for idx, baslik in enumerate(headers):
        if baslik in VARDIYALAR:
            ...
            # İlk açılışta otomatik vardiya seçili
            if key_id not in st.session_state:
                st.session_state[key_id] = (baslik == oto_vardiya_default)
    ...
    for i, baslik in enumerate(headers):
        ...
        if baslik in SIFIR_LIST:
            ...
            with col1:
                st.markdown(f"<div class='etiket'>{baslik}</div>", unsafe_allow_html=True)
            with col2:
                input_values[baslik] = st.number_input("Sayı", min_value=0, value=0, step=1, key=key_id, label_visibility="collapsed")
